...
<?php
include 'db.php';
if (isset($_GET['program_id']) && isset($_GET['yearlevel']) && $_GET['program_id']!="" && $_GET['yearlevel']!="") {
	$program_id = $_GET['program_id'];
	$yearlevel = $_GET['yearlevel'];
	$program_query = "SELECT m.materialID, p.program_id, p.programname, m.yearlevel, m.semester, m.materialtitle_, m.materialdocument from instructional m JOIN program p on p.program_id = m.programID where program_id = '$program_id' AND yearlevel = '$yearlevel'";
...
?>
	<section>
		<div class="header">
			...
			<div class="text-box">
				<h1><?php echo $programname; ?> <?echo $yearlevel;?></h1>
				</p>
				<hr>
	...
	<section class="title">
	<br> 
	<center><h1 class="past" style="text-transform: uppercase;">YEAR: <?php echo $yearlevel ?> YEAR</h1>
</center>
...
if (isset($_GET['program_id']) && isset($_GET['yearlevel']) && $_GET['program_id']!="" && $_GET['yearlevel']!="") {
	$program_id = $_GET['program_id'];
	$yearlevel = $_GET['yearlevel'];
		$program = "SELECT m.materialID, p.program_id, p.programname, m.yearlevel, m.semester, m.materialtitle_, m.materialdocument from instructional m JOIN program p on p.program_id = m.programID where program_id = '$program_id' AND yearlevel = '$yearlevel' GROUP BY semester";
...
<div class="card mb-3" style="max-width: 1200px; margin-left:100px">
  <div class="row g-0">
    <div class="col-md-4">
	<a href="instructionalsem_docu.php?program_id=<?php echo $row['program_id']; ?>&yearlevel=<?php echo $row['yearlevel'];?>&semester=<?php echo $row['semester']; ?>" ><img src="photos/folder.png" class="img-fluid rounded-start" alt="..." width="10%"></a>
