...
if (isset($_SESSION['username']) && isset($_SESSION['id'])) {
	...
	$facility = "SELECT * FROM facility order by name asc";
	$run = mysqli_query($con, $facility);
	$facilitytbl = mysqli_num_rows($run);
	?>
...
<div class="alert alert-success alert-dismissible fade show" role="alert">
  ...
  <a href="facility.php"><button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></a>
</div>
</div>
<?php } ?>
				...
				if (!empty($facilitytbl)) {
					While($row = mysqli_fetch_assoc($run)) {
						...
						<tr>
							...
							<?php echo "<td><img src='../admin/photos/".$row['picture']." 'width='60' height='60'/></td>" ?>
							<td> <?php echo $row['description']; ?> </td>
							<td><a href="facility_edit.php?facilityID=<?php echo $row['facilityID']; ?>"><button class='edit-administration'><i class='fa fa-pencil'></i></button></a><a href="crudadministration.php?facilityID=<?php echo $row['facilityID']; ?>"><button class='delete-administration'><i class='fa fa-trash'></i></button></a></td>
						</tr>
			<?php	}
				}
				?>
...
<div class="pop-up" id="add_administration_modal" tabindex="-1" role="dialog" aria-labeelledby="administration_modal" aria-hidden="true">
	<div class="overlay" role="document">
		...
		<form action = "crudadministration.php"method="post" enctype="multipart/form-data">
				<input type="text" name="name" placeholder="Facilty Name">
				<textarea name="description" placeholder="Facility Description"></textarea>
				<label for="image_">Image<small>(formats: jpg, jpeg, png)</small></label>
				<input type="file" name="picture" class="form-control">
				<button name="add_facility" class="add_save add-administration">Save</button>
		</form>
	</div>
</div>
