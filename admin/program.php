...
if (isset($_SESSION['username']) && isset($_SESSION['id'])) {
	Require'crud/pup.php';
			$program ="SELECT * FROM `program` order by prog_ac asc";
			$run = mysqli_query($con, $program);
			$programtbl = mysqli_num_rows($run);
	?>	
...
<div class="alert alert-success alert-dismissible fade show" role="alert" >
  ...
  <a href="program.php"><button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></a>
</div>
</div>
<?php } ?>
					...
					While($row = mysqli_fetch_assoc($run)) {
						?>
							<tr>
									...
									<?php echo "<td><img src='../admin/photos/".$row['prog_img']." 'width='60' height='60'/></td>" ?>
									<td><a href="program_edit.php?program_id=<?php echo $row['program_id']; ?>"><button class='edit-administration'><i class='fa fa-pencil'></i></button></a><a href="crudadministration.php?program_id=<?php echo $row['program_id']; ?>"><button class='delete-administration'><i class='fa fa-trash'></i></button></a></td>
...
<div class="pop-up" id="add_administration_modal" tabindex="-1" role="dialog" aria-labeelledby="administration_modal" aria-hidden="true">
	<div class="overlay" role="document">
		...
		<form action = "crudadministration.php"method="post" enctype="multipart/form-data">
				...
				<input type="text" name="prog_ac" placeholder="Program Accronym">
				...
				<input type="file" name="prog_img" class="form-control">

				<button name="add_program" class="add_save add-administration">Save</button>
		</form>
	</div>
</div>
