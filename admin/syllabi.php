...
$syllabi ="SELECT s.syllabiID, p.program_id, p.programname, s.syllabititle_, s.syllabi_img, s.syllabidocument from syllabi s JOIN program p on p.program_id = s.programID order by syllabiID asc";
$run = mysqli_query($con, $syllabi);
$syllabitbl = mysqli_num_rows($run);
?>	
...
<div class="alert alert-success alert-dismissible fade show" role="alert" style="margin-top:15px">
  ...
  <a href="syllabi.php"><button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></a>
</div>
</div>
<?php } ?>
					...
					While($row = mysqli_fetch_assoc($run)) {
						?>
							<tr>
									...
									<td><?php echo $row['syllabititle_']; ?></td>
									<?php echo "<td><img src='../admin/photos/".$row['syllabi_img']." 'width='60' height='60'/></td>" ?>
									<td><a href="syllabidoc.php?syllabiID=<?php echo $row['syllabiID']; ?>" target="_blank"><button class='pdf'><i class='fa fa-file-pdf-o'></i></button></a></td>
									<td><a href="syllabi_edit.php?syllabiID=<?php echo $row['syllabiID']; ?>"><button class='edit-administration'><i class='fa fa-pencil'></i></button></a><a href="crudadministration.php?syllabiID=<?php echo $row['syllabiID']; ?>"><button class='delete-administration'><i class='fa fa-trash'></i></button></a></td>
...
<div class="pop-up" id="add_administration_modal" tabindex="-1" role="dialog" aria-labeelledby="administration_modal" aria-hidden="true">
	<div class="overlay" role="document">
		...
		<form action = "crudadministration.php"method="post" enctype="multipart/form-data">
				...
				<input type="text" name="syllabititle_" placeholder="syllabi Title">
				<label for="office_img">syllabi Image<small>(formats: jpg, png, jpeg)</small></label>
				<input type="file" name="syllabi_img" class="form-control">
				<label for="office_img">syllabi Document<small>(formats: pdf)</small></label>
				<input type="file" name="syllabidocument" class="form-control">
				<button name="add_syllabi" class="add_save add-administration">Save</button>
		</form>
	</div>
</div>
