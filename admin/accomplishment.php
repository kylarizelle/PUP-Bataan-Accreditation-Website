...
if (isset($_SESSION['username']) && isset($_SESSION['id'])) {
			...
			$accomplishmenttbl = mysqli_num_rows($run);
	?>	
...
<div class="alert alert-success alert-dismissible fade show" role="alert">
  ...
  <a href="accomplishment.php"><button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></a>
</div>
<?php } ?>
					...
					While($row = mysqli_fetch_assoc($run)) {
						?>
							<tr>
									...
									<td><a href="accomplishment_edit.php?accom_id=<?php echo $row['accom_id']; ?>"><button class='edit-administration'><i class='fa fa-pencil'></i></button></a><a href="crudadministration.php?accom_id=<?php echo $row['accom_id']; ?>"><button class='delete-administration'><i class='fa fa-trash'></i></button></a></td>
...
<div class="pop-up" id="add_administration_modal" tabindex="-1" role="dialog" aria-labeelledby="administration_modal" aria-hidden="true">
	<div class="overlay" role="document">
		...
		<form action = "crudadministration.php"method="post" enctype="multipart/form-data">
				<input type="number" name="year_" placeholder="Accomplishment Year">
				<input type="text" name="achievement" placeholder="Achievement Name">
				<button name="add_accomplishment" class="add_save add-administration">Save</button>
		</form>
	</div>
</div>
