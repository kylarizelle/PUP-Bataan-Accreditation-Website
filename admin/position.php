...
if (isset($_SESSION['username']) && isset($_SESSION['id'])) {
		Require'crud/pup.php';
			...
			$run = mysqli_query($con, $position);
			$positiontbl = mysqli_num_rows($run);
	?>	
...
<div class="alert alert-success alert-dismissible fade show" role="alert">
  ...
  <a href="position.php"><button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></a>
</div>
</div>
<?php } ?>
...
<table id="table_id" class="display table table-striped table-bordered">
	<thead>
			...
			<tbody>
				...
				<?php 
				if (!empty($positiontbl)) {
					While($row = mysqli_fetch_assoc($run)) {
						?>
							<tr>
									<?php  $row['positionID']; ?>
									<td><?php echo $row['positionname']; ?></td>
									<td><a href="position_edit.php?positionID=<?php echo $row['positionID']; ?>"><button class='edit-administration'><i class='fa fa-pencil'></i></button></a><a href="crudadministration.php?positionID=<?php echo $row['positionID']; ?>"><button class='delete-administration'><i class='fa fa-trash'></i></button></a></td>
...
<div class="pop-up" id="add_administration_modal" tabindex="-1" role="dialog" aria-labeelledby="administration_modal" aria-hidden="true">
	<div class="overlay" role="document">
		...
		<form action = "crudadministration.php"method="post" enctype="multipart/form-data">
				<input type="text" name="positionname" placeholder="Position Name">
				<button name="add_position" class="add_save add-administration">Save</button>
		</form>
	</div>
</div>
