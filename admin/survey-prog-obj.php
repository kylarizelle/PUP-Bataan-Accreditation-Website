...
if (isset($_SESSION['username']) && isset($_SESSION['id'])) {
	Require'crud/pup.php';
			$surveyprogram ="SELECT s.surveyID, p.program_id, p.programname, s.objective FROM surveyprogram s JOIN program p on p.program_id = s.programID";
			$run = mysqli_query($con, $surveyprogram);
			$surveyprogramtbl = mysqli_num_rows($run);
	?>	
...
<div class="alert alert-success alert-dismissible fade show" role="alert">
  ...
  <a href="survey-prog-obj.php"><button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></a>
</div>
</div>
<?php } ?>
					...
					While($row = mysqli_fetch_assoc($run)) {
						?>
							<tr>
									...
									<td><a href="survey-prog-obj_edit.php?surveyID=<?php echo $row['surveyID']; ?>"><button class='edit-administration'><i class='fa fa-pencil'></i></button></a><a href="crudadministration.php?surveyID=<?php echo $row['surveyID']; ?>"><button class='delete-administration'><i class='fa fa-trash'></i></button></a></td>
...
<div class="pop-up" id="add_administration_modal" tabindex="-1" role="dialog" aria-labeelledby="administration_modal" aria-hidden="true">
	<div class="overlay" role="document">
		...
		<form action = "crudadministration.php"method="post" enctype="multipart/form-data">
				...
				<button name="add_surveyprog" class="add_save add-administration">Save</button>
		</form>
	</div>
</div>
